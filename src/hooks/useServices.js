...
export const useServices = () => {
  ...
  *,
  profiles!provider_id(${PUBLIC_PROFILE_FIELDS}),
  service_listings(price, unit)
`); // Admin approval is the public gate
  ...
  const fetchServiceGatedData = useCallback(
    async (id) => {
      if (!user) return null;
      try {
        const { data, error } = await supabase.rpc(
          "get_unlocked_service_details",
          { prov_id: id },
        );
        if (error) throw error;
        return data?.[0] || null;
