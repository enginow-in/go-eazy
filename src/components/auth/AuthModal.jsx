...
export const AuthModal = () => {
  ...
  const { authModalOpen, authModalTab, loginLockout } = useSelector(
    (s) => s.auth
  )
  ...
  const isLocked =
    loginLockout.locked && countdown > 0
  const warnAttempts =
    !isLocked &&
    loginLockout.attemptsRemaining <= 2 &&
    loginLockout.attemptsRemaining > 0
  ...
  return (
    ...
    >
          {/* Tabs */}
      <div
        className="flex gap-1 p-1 bg-gray-100 rounded-xl mb-4"
        role="tablist"
      ...
      {tab === 'login' && isLocked && (
        <div
          className="flex items-start gap-3 p-3 mb-4 rounded-xl bg-red-50 border border-red-200"
          role="alert"
        >
          <ShieldAlert
            size={18}
      ...
      {tab === 'login' && warnAttempts && (
        <div
          className="flex items-center gap-2 p-2.5 mb-4 rounded-lg bg-amber-50 border border-amber-200"
          role="alert"
        >
          <ShieldAlert
            size={15}
          ...
          <p className="text-xs font-semibold text-amber-700">
            Warning: {loginLockout.attemptsRemaining} attempt
            {loginLockout.attemptsRemaining !== 1 ? 's' : ''} remaining before
            lockout
          </p>
        </div>
      )}
      ...
      >
        ...
        <Input
          ...
          rightIcon={
            <button
              ...
              className="cursor-pointer rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#CA3433]"
            >
              {showPass ? (
        ...
        >
          {isLocked
            ? `Locked · ${Math.floor(countdown / 60)}:${String(
                countdown % 60
              ).padStart(2, '0')}`
            : tab === 'login'
            ? 'Sign In'
