...
import { Mail, Lock, User, Eye, EyeOff, Home, GraduationCap, Utensils, ArrowLeft } from 'lucide-react'
...
export const AuthModal = () => {
  ...
  const validate = () => {
    ...
    if (tab === 'signup') {
      // Sign Up requires the password to meet every strength rule, not just length.
      const strengthError = getPasswordErrorMessage(form.password)
      ...
      {tab === 'forgot' ? (
        <>
          <button
            ...
            className="flex items-center gap-1.5 text-sm font-semibold text-gray-500 hover:text-gray-700 mb-4"
          >
          ...
          {resetSent ? (
            <div className="text-center py-4">
              <p className="text-sm text-gray-600">
                If an account exists for <span className="font-semibold text-gray-900">{form.email}</span>,
        ...
        <>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              />
              {tab === 'signup' && <PasswordStrengthMeter password={form.password} />}
              {tab === 'login' && (
                <button
                  type="button"
            ...
            <Button
              ...
              loading={loading}
              disabled={tab === 'signup' && !evaluatePassword(form.password).isValid}
            >
