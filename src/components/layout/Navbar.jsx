        ...
        <div className="flex items-center justify-between h-20 relative">
          ...
          <div className="hidden md:flex items-center gap-6">
            <div className="flex items-center space-x-6 text-sm font-medium text-gray-500">
              ...
              <button onClick={() => user ? navigate(role === 'admin' ? '/systemadmin' : role === 'landlord' ? '/landlord' : role === 'service_provider' ? '/service-provider' : '/dashboard') : dispatch(openAuthModal('login'))} className="hover:text-gray-900 transition-colors">{t('nav.list')}</button>
       ...
       {mobileMenuOpen && (
        <div className="md:hidden absolute top-20 left-0 right-0 bg-white border-b border-gray-100 shadow-xl overflow-y-auto max-h-[80vh] z-50">
          <div className="px-4 py-4 space-y-4">
            ...
            <button onClick={() => { dispatch(closeMobileMenu()); user ? navigate(role === 'admin' ? '/systemadmin' : role === 'landlord' ? '/landlord' : role === 'service_provider' ? '/service-provider' : '/dashboard') : dispatch(openAuthModal('login')) }} className="block w-full text-left font-semibold text-gray-700 py-2">{t('nav.list')}</button>
