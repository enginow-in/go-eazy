...
export const NearbyServices = () => {
  ...
  const CATEGORIES = useMemo(() => [
    { value: '',         label: t('nearby.categories.all', 'All'), emoji: '✨' },
    { value: 'tiffin',  label: t('nearby.categories.tiffin', 'Tiffin'), emoji: '🍱' },
    { value: 'laundry', label: t('nearby.categories.laundry', 'Laundry'), emoji: '🧺' },
    { value: 'cleaning',label: t('nearby.categories.cleaning', 'Cleaning'), emoji: '🧹' },
  ], [t])

  const serviceSortOptions = useMemo(() => [
    { value: 'created_at:desc', label: t('nearby.sort.newest', 'Newest') },
    { value: 'created_at:asc',  label: t('nearby.sort.oldest', 'Oldest') },
    { value: 'views:desc',       label: t('nearby.sort.popular', 'Most popular') },
  ], [t])
  ...
  const renderFilterContent = () => (
    <div className="space-y-6">
      {/* Location Selection */}
      <div>
        <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 block">{t('nearby.locationSel', 'Location')}</h4>
        <div className="grid grid-cols-2 gap-3">
           <div className="flex flex-col gap-1.5">
             <div className="flex bg-gray-50 rounded-xl overflow-hidden border border-gray-200 focus-within:border-[#CA3433]/50 transition-colors pr-2">
               <input 
                 type="text" 
                 placeholder={t('nearby.cityPlaceholder', 'City')}
           ...
           <div className="flex flex-col gap-1.5">
             <div className="flex bg-gray-50 rounded-xl overflow-hidden border border-gray-200 focus-within:border-[#CA3433]/50 transition-colors pr-2">
               <input 
                 type="text" 
                 placeholder={t('nearby.areaPlaceholder', 'Area')}
                 className="w-full bg-transparent border-none text-sm py-2.5 px-3 focus:ring-0 outline-none" 
      ...
      {/* Sort By */}
      <div>
        <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 block">{t('nearby.sortBy', 'Sort by')}</h4>
      ...
      {/* Service Type */}
      <div>
        <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 block">{t('nearby.category', 'Category')}</h4>
      ...
      <div className="pt-4 flex gap-3 border-t border-gray-100">
        <Button variant="secondary" className="flex-1 bg-white hover:bg-gray-50 border border-gray-100 rounded-xl font-bold py-2.5 text-xs" onClick={() => { dispatch(resetServiceFilters()); setShowFilters(false); }}>{t('nearby.reset', 'Reset')}</Button>
        <Button variant="primary" className="flex-1 rounded-xl shadow-lg shadow-[#CA3433]/10 font-bold py-2.5 text-xs" onClick={applyFilters}>{t('nearby.showResults', 'Show results')}</Button>
      </div>
    </div>
  )
  // eslint-disable-next-line
    ...
    <div className="pt-6 pb-16 min-h-screen bg-gray-50/50">
      <div className="w-full px-0 sm:px-0 md:px-0 lg:px-0">
        ...
        <div className="px-2 sm:px-4 mb-4">
          ...
          >
            <ArrowLeft size={16} /> {t('nearby.backHome', 'Back home')}
          </button>
        </div>
        ...
        <div className="mb-4 px-2 sm:px-4 text-center">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-gray-900 tracking-tighter font-display uppercase">
            {t('nearby.header', 'Nearby')} <span className="text-[#CA3433]">{t('nearby.services', 'Services')}</span>
          </h1>
        </div>
        ...
        <div className="mb-4 px-2 sm:px-4 flex flex-col gap-4">
          <div className="flex flex-row gap-3 items-center w-full">
            ...
            <div className="relative shrink-0 flex items-center">
              ...
              >
                ...
                <div className="hidden sm:flex flex-col items-start pr-1 -space-y-0.5">
                  <span className="text-sm font-bold text-gray-950 leading-tight">{filters.city ? t(`cities.${filters.city}`, filters.city) : t('nearby.allCities', 'All cities')}</span>
                  <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest leading-none">{t('nav.state', 'State')}</span>
                </div>
                ...
                <>
                  ...
                  <div className="absolute left-0 top-full mt-3 w-64 bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-gray-100 z-50 max-h-[400px] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
                    <div className="p-4 border-b border-gray-50 bg-gray-50/50">
                      <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{t('nearby.selectCity', 'Select city')}</h4>
                    </div>
            ...
            <div className="relative flex-1">
              ...
              <input
                id="service-search"
                type="text"
                placeholder={t('nearby.searchPlaceholder', 'Search services')}
                value={searchInput}
          ...
          <div className="flex items-center gap-3 w-full">
            ...
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder={t('nearby.areaSearchPlaceholder', 'Search by area')}
                value={filters.area || ''}
            ...
            <div className="relative">
              ...
              >
                <Filter size={16} />
                <span className="hidden sm:inline">{t('nearby.filters', 'Filters')}</span>
        ...
        <div className="w-full border-t border-b border-gray-100 bg-white flex relative mb-4">
          ...
          <div className="flex items-center h-16 md:h-16 px-0 gap-4 md:gap-6 overflow-x-auto scrollbar-hide flex-1 scroll-smooth bg-white">
            ...
            >
              <Grid size={18} /> {t('nearby.categories.all', 'All')}
            </button>
        ...
        <div className="px-2 sm:px-4 mb-4 flex items-center justify-between gap-4">
          <p className="text-[10px] sm:text-sm text-gray-400 font-bold uppercase tracking-widest leading-none">
            {loading ? t('nearby.searching', 'Searching...') : t('nearby.providersFound', { count: services.length, defaultValue: `${services.length} providers found` })}
          </p>
        ...
        <div className="px-2 sm:px-4">
          ...
          <div className="bg-white rounded-3xl p-16 md:p-24 text-center border border-dashed border-gray-200 shadow-sm mx-4 sm:mx-10 md:mx-16 lg:mx-20">
            ...
            <p className="text-gray-500 max-w-xs mx-auto mb-8">{t('nearby.noFoundDesc', 'Try changing your search or filters.')}</p>
            <Button variant="secondary" onClick={() => { dispatch(resetServiceFilters()); setSearchInput('') }} className="rounded-xl border-gray-200">
              {t('nearby.reset', 'Reset')}
            </Button>
          </div>
        )}
      </div>
