...
import { ArrowLeft, X, Star, Check, Minus, Eye } from 'lucide-react'
...
export const PropertyCompare = () => {
        ...
        <div className="pt-8 pb-20 min-h-screen bg-gray-50">
          <div className="max-w-6xl mx-auto px-4">
            <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-gray-900 mb-6 transition-colors">
              <ArrowLeft size={16} /> {t('compare.browse')}
            </button>
            <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
              <div className="w-16 h-16 mx-auto mb-4 bg-gray-50 rounded-full flex items-center justify-center">
                <Eye size={32} className="text-gray-300" />
              </div>
              <h2 className="text-xl font-bold text-gray-900 mb-2">{t('compare.noProperties')}</h2>
              <p className="text-gray-500 mb-6">{t('compare.noPropertiesDesc')}</p>
  ...
  const rows = [
    { label: t('compare.price'), key: 'price', render: (p) => <span className="font-bold text-lg text-[#CA3433]">{formatPrice(p.price)}<span className="text-xs font-normal text-gray-400">/mo</span></span> },
    { label: t('compare.type'), key: 'type', render: (p) => <Badge variant={p.type === 'Room' ? 'brand' : p.type === 'Flat' ? 'success' : p.type === 'Hostel' ? 'warning' : 'purple'}>{p.type}</Badge> },
    { label: t('compare.bedrooms'), key: 'bedrooms', render: (p) => <span className="font-semibold">{p.bedrooms || '—'} BHK</span> },
    { label: t('compare.furnishing'), key: 'furnishing_type', render: (p) => {
    ...
    { label: t('compare.city'), key: 'city', render: (p) => <span className="font-semibold">{p.city}</span> },
    { label: t('compare.area'), key: 'area', render: (p) => <span className="text-sm text-gray-600">{p.area}</span> },
    { label: t('compare.availability'), key: 'availability', render: (p) => p.availability ? <span className="text-green-600 font-semibold flex items-center gap-1 justify-center"><Check size={14} /> {t('compare.available')}</span> : <span className="text-red-500 font-semibold">{t('compare.rented')}</span> },
    { label: t('compare.petFriendly'), key: 'pet_friendly', render: (p) => p.pet_friendly ? <span className="text-green-600"><Check size={16} /></span> : <span className="text-gray-300"><Minus size={16} /></span> },
    { label: t('compare.balcony'), key: 'balcony', render: (p) => p.balcony ? <span className="text-green-600"><Check size={16} /></span> : <span className="text-gray-300"><Minus size={16} /></span> },
    { label: t('compare.rating'), key: 'rating', render: (p) => {
    ...
    { label: t('compare.views'), key: 'views', render: (p) => <span className="font-semibold">{p.views}</span> },
  ]
    ...
    <>
      ...
      <div className="pt-8 pb-20 min-h-screen bg-gray-50">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <h1 className="text-2xl font-bold text-gray-900 font-display mb-8">{t('compare.title')}</h1>
          ...
          <div className="mt-8">
            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4">{t('compare.amenities')}</h3>
            <div className="grid gap-4" style={{ gridTemplateColumns: `180px repeat(${properties.length}, 1fr)` }}>
              <div />
              {properties.map((p, idx) => (
                <div key={idx} className="space-y-2">
                  ...
                  {(!p.amenities || p.amenities.length === 0) && (
                    <span className="text-sm text-gray-400">{t('compare.noAmenities')}</span>
                  )}
                </div>
              ))}
            </div>
