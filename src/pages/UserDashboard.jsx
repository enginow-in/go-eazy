...
export const UserDashboard = () => {
  ...
  // Separate abort controllers for dashboard data
  const createPropertiesController = useAbortController()
  const createUserDataController = useAbortController()

  const loadUserData = useCallback(async () => {
    if (!user) return
    const controller = createUserDataController()
    ...
    try {
      ...
      console.error('[UserDashboard] loadUserData error:', err)
    } finally {
      if (!controller.signal.aborted) {
        setLoadingData(false)
      }
    }
  }, [user, createUserDataController])

  const loadProperties = useCallback(async () => {
    if (!user) return

    const controller = createPropertiesController()
  ...
  }, [user, favorites, recentlyViewed, loadProperties, loadUserData])
  ...
  const LoadingRow = () => (
    <div className="flex gap-5 overflow-x-auto pb-4 scrollbar-hide">
      {[1, 2, 3, 4].map(i => (
        <div
          key={i}
          className="flex-shrink-0 w-64 rounded-xl bg-white border border-gray-100 p-3 space-y-3"
        >
              ...
              <div className="absolute top-full right-0 mt-3 w-80 sm:w-96 bg-white rounded-2xl shadow-[0_10px_40px_rgb(0,0,0,0.1)] border border-gray-100 p-4 z-50">
                <div className="flex items-center justify-between mb-3 border-b border-gray-50 pb-2">
                  ...
                  {unreadCount > 0 && (
                    <button
                      onClick={markAsRead}
                      className="text-xs font-bold text-[#CA3433] hover:underline px-2 py-1 rounded-md hover:bg-red-50 transition-colors"
                    >
                      Mark all read
                    </button>
                  )}
                      ...
                      <div
                        ...
                        className={`p-3 rounded-xl border transition-colors ${
                          n.is_read
                            ...
                            : 'bg-[#fffcf0] border-yellow-200 text-gray-900'
                        }`}
                      >
                        <p
                        ...
                        <p className="text-[10px] mt-2 text-gray-400 font-bold uppercase tracking-wider">
                          {new Date(
                            n.created_at
                          ).toLocaleDateString()}
                        </p>
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <h2 className="text-xl font-black text-gray-900 font-display leading-none">
                  Saved Properties
                </h2>
                {!loading && (
                  <span className="text-xs font-bold text-gray-400 mt-1 block uppercase tracking-wider">
                    {favProps.length} Items
                  </span>
                )}
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="bg-white p-10 rounded-3xl border border-gray-100 text-center shadow-sm">
              ...
              <Link
                to="/search"
                className="bg-[#fdf2f2] text-[#CA3433] px-6 py-2.5 rounded-xl font-bold hover:bg-[#fbe1e1] transition-colors inline-block"
              >
                Explore listings
              </Link>
            </div>
        ...
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              {!loadingData && (
                <span className="text-[10px] text-gray-400 mt-1 font-medium bg-gray-50 px-2 py-0.5 rounded-md inline-block uppercase tracking-wider">
          ...
          {loadingData ? (
            ...
            <div className="bg-white p-10 rounded-3xl border border-gray-100 text-center shadow-sm">
              ...
              <p className="text-gray-500 font-medium font-display">
                You haven't requested any site visits.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {myVisits.map(visit => (
                <div
                  key={visit.id}
                  className="bg-white border border-gray-100 rounded-xl p-4 flex flex-col gap-3 shadow-[0_2px_10px_rgb(0,0,0,0.02)] hover:shadow-[0_4px_16px_rgb(0,0,0,0.05)] transition-all cursor-pointer"
                  onClick={() => navigate(`/property/${visit.property_id}`)}
                >
                  <div className="flex justify-between items-start">
                    <div className="pr-2">
                      <h3 className="font-bold text-gray-900 line-clamp-1 text-[15px] hover:text-[#CA3433] transition-colors">
                        {visit.property?.title || 'Property Unavaliable'}
                      </h3>
