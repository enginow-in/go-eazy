...
export const UserDashboard = () => {
  ...
  // Bug Fix 1 & 2: Added cleanup lifecycle guards across asynchronous data loading operations
  useEffect(() => {
    let isMounted = true
    ...
    const fetchAllDashboardData = async () => {
      ...
      // 1. Fetch User Data (Notifications & Visits)
      try {
        setLoadingData(true)
      ...
      // 2. Fetch Property Lists (Favorites & Recently Viewed)
      ...
      try {
        ...
        if (favorites.length > 0) {
          ...
          if (data) {
            fetchedFavs = favorites.map(id => data.find(p => p.id === id)).filter(Boolean)
          }
        }
        ...
        if (recentlyViewed.length > 0) {
          ...
          if (data) {
            fetchedRecents = recentlyViewed.map(id => data.find(p => p.id === id)).filter(Boolean)
          }
        }

        if (isMounted) {
          setFavProps(fetchedFavs)
          setRecentProps(fetchedRecents)
          setLoading(false)
        }
      } catch (err) {
               ...
               {myVisits.map(visit => (
                 <div key={visit.id} className="bg-white border border-gray-100 rounded-xl p-4 flex flex-col gap-3 shadow-[0_2px_10px_rgb(0,0,0,0.02)] hover:shadow-[0_4px_16px_rgb(0,0,0,0.05)] transition-all cursor-pointer" onClick={() => navigate(`/property/${visit.property_id}`)}>
                   <div className="flex justify-between items-start">
                     <div className="pr-2">
                       <h3 className="font-bold text-gray-900 line-clamp-1 text-[15px] hover:text-[#CA3433] transition-colors">{visit.property?.title || 'Property Unavailable'}</h3>
