                  ...
                  <div className="space-y-3">
                    {service.contact_phone && (
                      <>
                        ...
                        <a
                          href={`https://wa.me/${(() => {
                            const rawPhone = gatedData?.contact_phone || service.contact_phone;
                            const cleaned = rawPhone.toString().replace(/\D/g, '');
                            return cleaned.length === 10 ? `91${cleaned}` : cleaned;
                          })()}?text=${encodeURIComponent(`Hi, I am interested in your service "${service.title}" listed on GoEazy. Can we connect?`)}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center justify-center gap-3 w-full px-5 py-3.5 rounded-full bg-[#25D366] text-white font-bold hover:bg-[#20ba5a] transition-colors text-[15px]"
                        >
