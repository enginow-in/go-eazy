...
export const Search = () => {
  ...
  const renderFilterContent = () => (
    <div className="space-y-6">
      {/* Location Selection */}
      <div>
        <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 block">
          Location Selection
        </h4>
          ...
          <div className="flex flex-col gap-1.5 focus-within:text-brand-600 transition-colors">
            ...
            <div className="flex bg-gray-50 rounded-xl overflow-hidden border border-gray-200 focus-within:border-brand-50-focus transition-colors pr-2">
              <input
                ...
                onChange={(e) =>
                  setLocalFilters((prev) => ({ ...prev, area: e.target.value }))
                }
              />
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">
              Price Max
            </h4>
      ...
      <div className="pt-4 flex gap-3 border-t border-gray-100">
        <Button
          variant="secondary"
          className="flex-1 bg-white hover:bg-gray-50 border border-gray-100 rounded-xl font-bold py-2.5"
          onClick={() => {
        ...
        <Button
          variant="primary"
          className="flex-1 rounded-xl shadow-lg shadow-brand-500/10 font-bold py-2.5"
  ...
  const filterContent = useMemo(
    () => renderFilterContent(),
    [localFilters, t, dispatch, showFilters],
  );
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            ...
            <div className="flex items-center justify-between gap-4 w-full">
              <div>
                <h1 className="text-[14px] sm:text-2xl md:text-3xl font-light text-gray-800 flex items-center gap-2 italic font-display tracking-tight pr-2">
                  <span className="text-brand-500 font-normal opacity-50">
                    —
                  </span>
                  <span>
                    {t("search.quoteStart")}{" "}
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 text-gray-400 mr-2">
              <button
                onClick={() => setViewMode("grid")}
                className={`hover:text-gray-900 transition-colors ${viewMode === "grid" ? "text-gray-900 border border-gray-200 rounded p-1 shadow-sm" : "p-1"}`}
              >
                <Grid size={20} />
              </button>
              <button
                onClick={() => setViewMode("list")}
                className={`hover:text-gray-900 transition-colors ${viewMode === "list" ? "text-gray-900 border border-gray-200 rounded p-1 shadow-sm" : "p-1"}`}
              >
            ...
            <div className="relative z-20">
              ...
              >
                ...
                <ChevronDown
                  size={14}
                  className={`ml-2 transition-transform duration-300 ${showFilters ? "rotate-180 text-brand-500" : "text-gray-400"}`}
                />
              </button>
        ...
        {loading && listings.length === 0 ? (
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-6 xl:gap-8">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div
                key={i}
                className="bg-white rounded-xl border border-gray-100/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] space-y-4 overflow-hidden"
              >
          ...
          <>
            ...
            >
              {listings.map((p) => (
                <PropertyCard key={p.id} property={p} layout={viewMode} />
              ))}
            </div>
            {hasMore && (
