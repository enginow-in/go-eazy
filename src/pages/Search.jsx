...
export const Search = () => {
  ...
  const renderFilterContent = () => (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
           <div className="flex flex-col gap-1.5 focus-within:text-brand-600 transition-colors">
             ...
             <div className="flex bg-gray-50 dark:bg-slate-800 rounded-xl overflow-hidden border border-gray-200 dark:border-slate-700 focus-within:border-brand-50-focus transition-colors pr-2">
               <input 
                 type="text" 
                 id="filter-city"
                 name="city"
                 placeholder="City (e.g. Dehradun)" 
                 className="w-full bg-transparent border-none text-sm py-2.5 px-3 focus:ring-0 outline-none dark:text-white" 
          ...
          <div className="grid grid-cols-2 gap-2">
            {SORT_OPTIONS.map(opt => (
              <button
                ...
                className={`px-3 py-2 rounded-xl text-[11px] font-semibold transition-all border ${localFilters.sortBy + ':' + localFilters.sortOrder === opt.value ? 'bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400 border-brand-200 dark:border-brand-500/30 shadow-sm' : 'border-gray-100 dark:border-slate-800 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}
              >
                {opt.label}
              </button>
            ))}
        ...
        <div className="flex flex-wrap gap-2">
          {['Room', 'Flat', 'Hostel', 'PG'].map(type => (
            <button
              ...
              className={`px-5 py-2 rounded-xl text-[13px] font-semibold transition-all border ${localFilters.type === type ? 'bg-[#fdf2f2] dark:bg-brand-500/10 text-[#CA3433] dark:text-brand-400 border-[#fbe1e1] dark:border-brand-500/30 shadow-sm' : 'border-gray-100 dark:border-slate-800 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}
            >
      ...
      <div className="pt-4 flex gap-3 border-t border-gray-100 dark:border-slate-850">
        <Button variant="secondary" className="flex-1 bg-white dark:bg-slate-900 hover:bg-gray-50 dark:hover:bg-slate-800 border border-gray-100 dark:border-slate-800 rounded-xl font-bold py-2.5 dark:text-white" onClick={() => { dispatch(resetFilters()); setShowFilters(false); }}>Reset All</Button>
  ...
  return (
    <div className="pt-4 pb-12 min-h-screen bg-gray-50/50 dark:bg-brand-dark">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div className="flex-1 w-full">
            <div className="flex items-center justify-between gap-4 w-full">
              <div>
                <h1 className="text-[14px] sm:text-2xl md:text-3xl font-light text-gray-800 dark:text-gray-100 flex items-center gap-2 italic font-display tracking-tight pr-2">
                  <span className="text-brand-500 font-normal opacity-50">—</span> 
                  <span>{t('search.quoteStart')} <strong className="font-extrabold text-gray-900 dark:text-white">{t('search.quoteEnd')}</strong>"</span>
                </h1>
              ...
              <div className="md:hidden flex items-center gap-3 relative z-30">
                <div className="flex items-center gap-2 pr-2 border-r border-gray-200 dark:border-slate-800 mr-1">
                  <button 
                    onClick={() => setViewMode('grid')} 
                    className={`p-2 transition-all rounded-lg ${viewMode === 'grid' ? 'bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400 ring-1 ring-brand-100 dark:ring-brand-500/30' : 'text-gray-400'}`}
                  >
                    <Grid size={18} />
                  </button>
                  <button 
                    onClick={() => setViewMode('list')} 
                    className={`p-2 transition-all rounded-lg ${viewMode === 'list' ? 'bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400 ring-1 ring-brand-100 dark:ring-brand-500/30' : 'text-gray-400'}`}
                  >
                ...
                <div className="relative">
                  <button 
                    ...
                    className={`flex items-center justify-center p-2.5 bg-white dark:bg-slate-900 border rounded-xl transition-all shadow-sm ${showFilters ? 'border-brand-500 text-brand-600 ring-2 ring-brand-50' : 'border-gray-200 dark:border-slate-800 text-gray-700 dark:text-gray-300 hover:border-gray-300'}`}
                  ...
                  {showFilters && (
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-3 w-[calc(100vw-2rem)] xs:w-[340px] bg-white dark:bg-slate-900 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-gray-100 dark:border-slate-800 p-5 z-20 cursor-default animate-in fade-in zoom-in-95 duration-200">
                        {filterContent}
                      </div>
                    </>
                  )}
          ...
          <div className="hidden md:flex items-center gap-4">
             ...
             <div className="flex items-center gap-3 text-gray-400 mr-2">
               <button onClick={() => setViewMode('grid')} className={`hover:text-gray-900 dark:hover:text-white transition-colors ${viewMode === 'grid' ? 'text-gray-900 dark:text-white border border-gray-200 dark:border-slate-800 rounded p-1 shadow-sm' : 'p-1'}`}>
                 <Grid size={20} />
               </button>
               <button onClick={() => setViewMode('list')} className={`hover:text-gray-900 dark:hover:text-white transition-colors ${viewMode === 'list' ? 'text-gray-900 dark:text-white border border-gray-200 dark:border-slate-800 rounded p-1 shadow-sm' : 'p-1'}`}>
             ...
             <div className="relative z-20">
               <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-6 py-2.5 bg-white dark:bg-slate-900 border rounded-xl text-sm font-semibold transition-all ml-4 ${showFilters ? 'border-brand-500 text-brand-600 shadow-sm' : 'border-gray-200 dark:border-slate-800 text-gray-700 dark:text-gray-300 hover:shadow-sm'}`}>
                  <Filter size={16} />
               ...
               {showFilters && (
                 <>
                   ...
                   <div className="absolute right-0 top-full mt-3 w-[460px] bg-white dark:bg-slate-900 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-100 dark:border-slate-800 p-6 z-20 cursor-default overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                     {filterContent}
                   </div>
                 </>
               )}
