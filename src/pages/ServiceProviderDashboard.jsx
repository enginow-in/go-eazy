...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full ${conf.cls}`}
    >
...
export const ServiceProviderDashboard = () => {
  ...
  const handlePayToGoLive = async (serviceId) => {
    ...
    try {
      ...
      if (!orderResp.ok) {
        ...
        throw new Error(
          errJson.error || `Order creation failed (HTTP ${orderResp.status})`,
        );
      }
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async function (response) {
          try {
            ...
            toast.success(
              "🎉 Payment successful! Your listing is now LIVE on GoEazy.",
            );
          } catch (err) {
            toast.error(
              "Payment recorded but DB update failed. Contact support.",
            );
          } finally {
            setPayingId(null);
          }
  ...
  const verified = myServices.filter(
        ...
        {loading ? (
            ...
            {myServices.map((service) => {
              ...
              return (
                ...
                >
                  {/* Category Icon */}
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl shrink-0 ${cat.color || "bg-gray-100"} border border-gray-100`}
                  >
                    {cat.emoji}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-gray-400">
                      <span className="flex items-center gap-1">
                        <Eye size={11} /> {service.views || 0} views
                      </span>
                      <span className="flex items-center gap-1">
                        <Package size={11} />{" "}
                        {service.service_listings?.length || 0} items
                      </span>
          ...
          <div className="text-center py-24 bg-white rounded-2xl border border-gray-100">
            ...
            <p className="text-sm text-gray-500 max-w-xs mx-auto">
              Create your first service listing using the button above and start
