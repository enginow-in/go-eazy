...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Use service role key to validate ES256 JWTs and bypass RLS for updates
    ...
    // 2. Verify service ownership — the service must belong to this user
    ...
    if (serviceError || !service) {
      return new Response(JSON.stringify({ error: 'Service listing not found' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 404,
      })
    }

    if (service.provider_id !== user.id) {
      return new Response(JSON.stringify({ error: 'You do not own this service listing' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 403,
      })
    }

    if (service.payment_status === 'paid') {
      return new Response(JSON.stringify({ error: 'This service listing is already paid' }), {
    ...
    if (!isValid) {
      console.error('HMAC verification failed for service payment:', { service_id, razorpay_order_id })
      return new Response(JSON.stringify({ error: 'Invalid payment signature' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 403,
      })
    }

    // 4. Cross-validate with Razorpay API — ensure amount is exactly ₹199
    ...
    // 5. All checks pass — update payment_status using admin client (bypasses RLS)
    ...
    if (updateError) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to activate listing: ' + updateError.message }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 500,
      })
    }

    console.log(`Service payment verified: service=${service_id}, payment=${razorpay_payment_id}, user=${user.id}`)

    return new Response(JSON.stringify({ success: true, service_id }), {
